    ...
    <tbody>
        ...
        foreach ($roUsuario->result() as $oUsuario) {
            ...
            <tr>
                ...
                <td class="hidden-xs hidden-sm"><?php echo dataHelper::converteDataParaPagina($oUsuario->data_cadastro) ?></td>
                <td class="opcoes">
                    <?php
                    ...
                    if (isset($vPainelPermissao['painel/usuario/remover'])) {
                        ?>
                        <a class="btn btn-danger" href="javascript:;" onclick="confirmacao('Você tem certeza que deseja remover este item?', '<?php echo base_url(); ?>painel/usuario/remover/<?php echo $oUsuario->id; ?>');"><i class="icon-trash"></i> <span class="hidden-xs">Excluir</span></a>
                        <?php
                    }
                    ?>
                </td>
